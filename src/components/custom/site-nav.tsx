...
// better-auth ships `useSession()` (via the installed auth module), but that
// hook throws during Next.js's SSR pass because it calls `React.useRef` before
// the React tree is fully wired on the server. The shared user-owned
// `useMountedSession` reads it once on mount instead.
function useIsAuthenticated(): boolean {
  return useMountedSession().data !== null;
}
