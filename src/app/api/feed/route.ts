...
//
// requireAuth gates access. Returns one page of scored profiles from the
// canonical discovery candidate pool, excluding its safety, Swipe, recent
// 'seen', and outgoing Connection targets. Cursor = last profile.id; ordering
// = score DESC, profile.id ASC for stable pagination. If the viewer hasn't
// filled in their profile, returns an empty `items` with `hasProfile: false`
// so the page can render a "edit your profile first" CTA.
